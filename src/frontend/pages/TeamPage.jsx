...
export default function TeamPage() {
  ...
  async function loadTeam() {
    setLoading(true);
    try {
      const res  = await authFetch("http://localhost:8000/teams/my-team");
  ...
  async function handleCreateTeam(e) {
    ...
    try {
      const res  = await authFetch("http://localhost:8000/teams/create", {
        method: "POST",
  ...
  async function handleAddMember(e) {
    ...
    try {
      const res  = await authFetch("http://localhost:8000/teams/add-member", {
        method: "POST",
  ...
  async function handleRemoveMember(memberEmail) {
    ...
    try {
      const res  = await authFetch("http://localhost:8000/teams/remove-member", {
        method: "POST",
  ...
  async function handlePromote(memberEmail) {
    ...
    try {
      const res  = await authFetch("http://localhost:8000/teams/promote-manager", {
        method: "POST",
        body:   JSON.stringify({ email: memberEmail }),
      });
      const data = await res.json();
  ...
  async function handleClaimManager() {
    ...
    try {
      const res  = await authFetch("http://localhost:8000/auth/claim-manager", { method: "POST" });
      const data = await res.json();
