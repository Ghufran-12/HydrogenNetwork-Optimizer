...
export default function ManagerDashboardPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res  = await authFetch("https://hydrogennetwork-optimizer-1.onrender.com/manager/dashboard");
  ...
  async function handleDelete(targetId) {
    ...
    try {
      const res  = await authFetch(`https://hydrogennetwork-optimizer-1.onrender.com/targets/${targetId}`, { method: "DELETE" });
      const json = await res.json();
